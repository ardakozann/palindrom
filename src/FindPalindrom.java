public class FindPalindrom {
	...
	String palindromeTamamlayiciSayi(String sayi) {
		...
		if (firstTimeCheckIfPalindrome(sayi)) {
			return "0";
		}
		if (sayi.length() == 2) {
		...
		// Recursive yapıdan dolayı önceki metottan kalanları toplamak için
		// fromBeforeTransaction değişkeni yaptım.
		...
		for (int i = 0; i < temp.length() / 2; i++) {
			...
			// Eğer sol yarımdaki rakam sağ yarımdaki rakamdan küçükse
				...
				// 81. satırdaydı buraya aldım.
				// problem ise aradaki fark resulta atılıyor ancak bir daha metota gönderilirken
				// temp değiştirilmeden gönderiliyor.
				result.setCharAt(temp.length() - 1 - i, intToChar(temp1));
		...
		// uzunluğu çift olan (!) ve sol yarımın en sağındaki sayı 1 artırılırsa onu
		// kontrol etmiyor. Onun için elle yapıyorum. Eğer sol yarımın en sağındaki
		// rakam orjinaline göre değişmişse cevap olarak döndüreceğim palindrom olması
		// için gereken sayının aynı indeksteki değerine 1 ekliyorum.
